    ...
    if (isset($_FILES['svg_file']) && $_FILES['svg_file']['error'] === 0) {
        ...
        if ($file_ext !== 'svg') {
            ...
            if ($svg_content === false) {
                ...
                // SECURITY: Remove XML declaration
                $svg_content = preg_replace('/<\?xml.*?\?>/i', '', $svg_content);
                
                // SECURITY: Remove DOCTYPE
                ...
                // SECURITY: Remove embedded styles (so CSS can style it)
                $svg_content = preg_replace('/<style\b[^<]*(?:(?!<\/style>)<[^<]*)*<\/style>/gi', '', $svg_content);
                
                // MINIFY: Remove excessive whitespace and newlines
                ...
                if (empty($svg_content_escaped)) {
                    $message = '<div class="alert alert-danger">SVG content is empty after processing.</div>';
                    ErrorLogger::log("SVG content empty after processing for: $name", 'ERROR');
                } else {
                    ...
                    if (file_put_contents($upload_path, $svg_content) !== false) {
                        if (isset($_POST['id']) && $_POST['id']) {
                            ...
                            if ($conn->query($update_query)) {
                                ...
                                ErrorLogger::log("Icon updated: ID $id, SVG content length: " . strlen($svg_content_escaped), 'INFO');
                            } else {
                            ...
                            if ($conn->query($insert_query)) {
                                ...
                                ErrorLogger::log("Icon created: $name, SVG content length: " . strlen($svg_content_escaped), 'INFO');
                            } else {
